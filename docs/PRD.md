...
> Audience: humans and AI tools contributing to this project. This document describes *what* the
...
- **Learner** — the only user role in v1. Can browse courses, enroll, watch lessons, track
  progress, and download certificates.

There is no in-app content-management/admin role in v1 (see [Section 8](#8-out-of-scope--future-considerations)).
...
- Minimum certificate content: learner name, course title, and completion date. *(Open question —
  see [Section 9](#9-open-questions).)*
...
- **Enrollment** — links a User to a Course; exists once the learner subscribes; tracks
  enrollment date.
...
- **Reliability**: progress tracking must accurately reflect actual watch behavior and must not
  lose recorded progress.
...
1. **Content entry process**: v1 has no admin UI. How will courses and lessons actually be added
   to the catalog before such a UI exists? To be defined as part of the technical plan (e.g., a
   seeding/content process), not by inventing a mechanism here.
2. **Certificate fields**: beyond learner name, course title, and completion date, should the
   certificate include a unique verification/serial code, instructor name, or course duration?
3. **Completion threshold**: exact watch-percentage required to mark a lesson complete is not yet
   defined (e.g., 90%, 95%, 100%).
